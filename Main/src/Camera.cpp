...
static float PitchScaleFunc(float input)
{
    ...
    if (input < kLower)
        ...
        scaled = ((input - kUpper) / (12 - kUpper)) * (12 - uUpper) + uUpper;
...
static Transform GetOriginTransform(float pitch, float offs, float roll)
{
    if (g_aspectRatio < 1.0f)
    {
        ...
        auto anchor = Transform::Translation({ offs, -0.8f, 0 }) * Transform::Rotation({ 1.5f, 0, 0 });
        auto contnr = Transform::Translation({ 0, 0, -0.9f }) * Transform::Rotation({
                                                                  -90 + pitch,
                                                                  0,
                                                                  0,
            });
    ...
    else
    {
        ...
        auto anchor = Transform::Translation({ offs, -0.9f, 0 }) * Transform::Rotation({ 1.5f, 0, 0 });
        auto contnr = Transform::Translation({ 0, 0, -0.9f }) * Transform::Rotation({
                                                                  -90 + pitch,
                                                                  0,
                                                                  0,
...
void Camera::Tick(float deltaTime, class BeatmapPlayback& playback)
{
    ...
    if (m_spinProgress < 2.0f)
    {
        ...
        else if (m_spinType == SpinStruct::SpinType::Bounce)
        {
            ...
            m_spinBounceOffset = DampedSin(m_spinProgress / 2, m_spinBounceAmplitude,
                m_spinBounceFrequency / 2, m_spinBounceDecay) *
                m_spinDirection;
        }
    ...
    auto GetZoomedTransform = [&](Transform t)
    {
        ...
        float zoomAmt;
        if (pLaneZoom <= 0)
            zoomAmt = pow(ZOOM_POW, -pLaneZoom) - 1;
        else
            zoomAmt = highwayDist * (pow(ZOOM_POW, -pow(pLaneZoom, 1.35f)) - 1);
