...
void nk_multiline_label(struct nk_context* ctx, const char* allText, nk_text_alignment ali, float width)
{
    ...
    do
    {
        ...
        do
        {
            ...
            while (
                font->width(font->userdata, font->height, text, textEnd) > width && textEnd > 1)
            {
...
void BasicPrompt::DrawWindow()
{
    ...
    if (m_shrinkWindow)
    {
        nk_window_set_size(m_nctx, *m_name, nk_vec2(static_cast<float>(m_width), end_pos.y - start_pos.y + 60.0f));
