...
static void PopulateScoreJSON(nlohmann::json& json, const ScoreIndex& score, const BeatmapSettings& map)
{
    json["score"] = {
        ...
        {"error", score.miss},
        {"gauge", score.gauge},
        {"options", {{"gaugeType", score.gaugeType}, {"gaugeOpt", score.gaugeOption}, {"mirror", score.mirror}, {"random", score.random}, {"autoFlags", score.autoFlags}}},
        {"timestamp", score.timestamp},
        {"windows", {{"perfect", score.hitWindowPerfect}, {"good", score.hitWindowGood}, {"hold", score.hitWindowHold}, {"miss", score.hitWindowMiss}, {"slam", score.hitWindowSlam}}} };
...
namespace IR
{
    ...
    {
        if (json.find("statusCode") == json.end())
            return false;
        if (json["statusCode"] < 20 || json["statusCode"] > 59)
            return false;
    ...
    {
        if (!ValidateReturn(json))
            return false;

        if (json["statusCode"] == 20)
        {
            if (json["body"].find("adjacentAbove") == json["body"].end())
                return false;
            if (!json["body"]["adjacentAbove"].is_array())
                return false;

            if (json["body"].find("adjacentBelow") == json["body"].end())
                return false;
            if (!json["body"]["adjacentAbove"].is_array())
                return false;
            ...
            if (json["body"].find("isPB") == json["body"].end())
                return false;
            if (!json["body"]["isPB"].is_boolean())
                return false;

            if (json["body"].find("isServerRecord") == json["body"].end())
                return false;
            if (!json["body"]["isServerRecord"].is_boolean())
                return false;

            if (json["body"].find("score") == json["body"].end())
                return false;
            if (!json["body"]["score"].is_object())
                return false;

            if (json["body"].find("serverRecord") == json["body"].end())
                return false;
            if (!json["body"]["serverRecord"].is_object())
                return false;
        }
