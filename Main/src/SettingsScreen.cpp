...
private:
    ...
    inline void RenderKeyBindings()
    {
        ...
        if (nk_group_begin(m_nctx, "Key Bindings Main", NK_WINDOW_NO_SCROLLBAR))
        {
            ...
            if (nk_button_label(m_nctx, m_controllerButtonNames[0].c_str()))
                OpenButtonBind((*m_activeBTKeys)[0]);
            ...
            if (nk_button_label(m_nctx, m_controllerButtonNames[1].c_str()))
                OpenButtonBind((*m_activeBTKeys)[1]);
            if (nk_button_label(m_nctx, m_controllerButtonNames[2].c_str()))
                OpenButtonBind((*m_activeBTKeys)[2]);
            if (nk_button_label(m_nctx, m_controllerButtonNames[3].c_str()))
                OpenButtonBind((*m_activeBTKeys)[3]);
            if (nk_button_label(m_nctx, m_controllerButtonNames[4].c_str()))
                OpenButtonBind((*m_activeBTKeys)[4]);
            ...
            if (nk_button_label(m_nctx, m_controllerButtonNames[5].c_str()))
                OpenButtonBind((*m_activeBTKeys)[5]);
            if (nk_button_label(m_nctx, m_controllerButtonNames[6].c_str()))
                OpenButtonBind((*m_activeBTKeys)[6]);
            ...
            LayoutRowDynamic(1);
            Label("Back:");
            if (nk_button_label(m_nctx, m_controllerButtonNames[7].c_str()))
                OpenButtonBind((*m_activeBTKeys)[7]);
            ...
            if (!m_useBTGamepad)
            {
                ...
                if (nk_option_label(m_nctx, "Primary", m_altBinds ? 0 : 1) > 0)
                    m_altBinds = false;
                if (nk_option_label(m_nctx, "Alternate", m_altBinds ? 1 : 0) > 0)
                    m_altBinds = true;
            }
    ...
    inline void RenderLaserSensitivitySettings()
    {
        ...
        if (nk_button_label(m_nctx, "Calibrate Laser Sensitivity"))
            OpenCalibrateSensitivity();
    }
    ...
    inline void UpdateInputKeyBindingStatus()
    {
        ...
        if (m_altBinds)
            m_activeLaserKeys = &m_altKeyboardLaserKeys;
        else
            m_activeLaserKeys = &m_keyboardLaserKeys;
    }
...
private:
    ...
    void RenderLaserColorSetting()
    {
        ...
        // Color
        if (nk_button_color(m_nctx, leftColor))
            m_laserColorPaletteVisible = !m_laserColorPaletteVisible;
        if (nk_button_color(m_nctx, rightColor))
            m_laserColorPaletteVisible = !m_laserColorPaletteVisible;
        ...
        // Text
        {
            ...
            if (lcolIntNew != lcolInt)
                m_laserColors[0] = static_cast<float>(lcolIntNew);

            const int rcolIntNew = IntInput(rcolInt, "Right hue", 0, 360);
            if (rcolIntNew != rcolInt)
                m_laserColors[1] = static_cast<float>(rcolIntNew);
        }
...
public:
    ButtonBindingScreen_Impl(GameConfigKeys key, bool gamepad, int controllerIndex, bool isAlt)
    {
        ...
        case GameConfigKeys::Controller_Laser0Axis:
            m_keyName = "Left Laser";
            break;
        case GameConfigKeys::Controller_Laser1Axis:
            m_keyName = "Right Laser";
            break;
        default:
    ...
    void OnKeyPressed(SDL_Scancode code, int32 delta) override
    {
        ...
        else if (!m_isGamepad && m_knobs)
        {
            ...
            {
            case GameConfigKeys::Controller_Laser0Axis:
                g_gameConfig.Set(m_completed ? m_isAlt ? GameConfigKeys::Key_Laser0PosAlt : GameConfigKeys::Key_Laser0Pos : m_isAlt ? GameConfigKeys::Key_Laser0NegAlt
                    : GameConfigKeys::Key_Laser0Neg,
                    code);
                break;
            case GameConfigKeys::Controller_Laser1Axis:
                g_gameConfig.Set(m_completed ? m_isAlt ? GameConfigKeys::Key_Laser1PosAlt : GameConfigKeys::Key_Laser1Pos : m_isAlt ? GameConfigKeys::Key_Laser1NegAlt
                    : GameConfigKeys::Key_Laser1Neg,
                    code);
                break;
            default:
