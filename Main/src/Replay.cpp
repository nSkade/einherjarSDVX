...
void Replay::UpdateToTime(MapTime lastTime)
{
    ...
    for (; m_nextJudgementIndex < m_judgementEvents.size(); m_nextJudgementIndex++)
    {
        ...
        MapTime time = (m_type == ReplayType::Legacy ? j->time : j->GetHitTime());
...
{
    if (s->score != this->score || s->crit != this->crit || s->almost != this->almost || s->miss != this->miss || s->gauge != this->gauge)
    {
        return false;
    }
    return true;
