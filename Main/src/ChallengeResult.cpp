...
public:
        ...
        for (unsigned int i = 0; i < res.size(); i++)
        {
            ...
            for (size_t i = 0; i < cres.scorescreenInfo.simpleNoteHitStats.size(); ++i)
            {
                ...
                m_PushFloatToTable("timeFrac",
                    Math::Clamp(static_cast<float>(simpleHitStat.time) / (cres.scorescreenInfo.beatmapDuration > 0 ? cres.scorescreenInfo.beatmapDuration : 1),
                        0.0f, 1.0f));
    ...
    void Tick(float deltaTime) override
    {
        if (!m_hasScreenshot && m_hasRendered && !IsSuspended())
        {
            ...
            if (screensetting == AutoScoreScreenshotSettings::Always || (screensetting == AutoScoreScreenshotSettings::Highscore && m_manager->GetOverallResults().goodScore))
            {
                Capture();
            }
