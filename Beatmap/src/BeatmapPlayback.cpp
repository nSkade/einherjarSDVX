...
bool BeatmapPlayback::Reset(MapTime initTime, MapTime start)
{
    ...
    if (start <= 0)
        start = std::numeric_limits<decltype(start)>::min();
...
void BeatmapPlayback::Update(MapTime newTime)
{
    ...
    if (objEnd != m_currObject)
    {
        for (auto it = m_currObject; it < objEnd; it++)
        {
            ...
            if (!m_playRange.Includes(obj->time))
                continue;
            if (obj->type == ObjectType::Hold && !m_playRange.Includes(obj->time + obj->hold.duration, true))
                continue;
    ...
    if (objEnd != m_currLaserObject)
    {
        for (auto it = m_currLaserObject; it < objEnd; it++)
        {
            ...
            if (!m_playRange.Includes(obj->time))
                continue;
            if (!m_playRange.Includes(obj->time + obj->laser.duration, true))
                continue;
    ...
    auto ObjectStateToStr = [](const ObjectState* state)
    {
        ...
        case ObjectType::Laser:
        {
            ...
            return Utility::Sprintf("%d(type=laser %s len=%d start=%.3f end=%.3f flags=%x)",
                obj->time, state->index == 0 ? "left" : state->index == 1 ? "right"
                : "invalid",
                state->duration, state->points[0], state->points[1], static_cast<unsigned int>(state->flags));
        }
        break;
        case ObjectType::Event:
        {
