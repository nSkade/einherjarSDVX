...
void BaseGameSettingsDialog::Tick(float deltaTime)
{
	...
	for (int i=0;i<2;++i) {
		if (m_isRepeat[i]) {
			...
			if (m_repeatTimer[i] > m_repeatTimeStart) {
				m_ChangeStepSetting(i*2-1);
			}
		}
	}
	for (int i=2;i<4;++i) {
...
bool BaseGameSettingsDialog::Init()
{
	...
	g_gameWindow->OnKeyReleased.Add(this, &BaseGameSettingsDialog::m_OnKeyReleased);
...
void BaseGameSettingsDialog::m_OnKeyReleased(SDL_Scancode code, int32 delta)
{
	if (!m_active || m_closing)
		return;
