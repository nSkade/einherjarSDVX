...
void FastGuiGame::Render(float deltaTime)
{
    ...
    if (m_combo)
    {
        ...
        t *= Transform::Translation({ static_cast<float>(g_resolution.x - m_comboText->size.x * easedScale) / 2.0f,
    ...
    for (size_t i = 0; i < 2; i++)
    {
        ...
        t *= Transform::Translation({ g_resolution.x / 2.0f + g_resolution.x * 0.3f * (i == 0 ? -1 : 1) - (w * m_scale) / 2.0f,
        ...
        p.SetParameter("color", Color::White);
        textTransform *= Transform::Translation({ m_scale * w * 0.5f - m_alertText[i]->size.x * 0.5f,
