...
const Map<String, std::pair<String, String>> ChallengeIndex::ChallengeDescriptionStrings = {
    // Overrideable reqs
    {"clear", {"Clear all charts", "Clear this chart"}},
    {"excessive clear", {"Clear all charts on Excessive Rate", "Clear this chart on Excessive Rate"}},
    {"permissive clear", {"Clear all charts on Permissive Rate", "Clear this chart on Permissive Rate"}},
    {"blastive clear", {"Clear all charts on Blastive %.1f* Rate", "Clear this chart on Blastive %.1f* Rate"}},
    {"permissive clear", {"Play on Permissive Rate", ""}},
    {"min_percentage", {"Get %d%% completion on each chart", "Get %d%% completion"}},
    {"min_gauge", {"Get %d%% gauge on each chart", "Get %d%% gauge"}},
    {"max_errors", {"Get less than %d errors per chart", "Get less than %d errors"}},
    {"max_nears", {"Get less than %d nears per chart", "Get less than %d nears"}},
    {"min_crits", {"Get at least %d crits per chart", "Get at least %d crits"}},
    {"min_chain", {"Get a chain at least %d long each chart", "Get a chain at least %d long"}},

    // Global reqs
    {"min_average_percentage", {"Get %d%% completion overall", ""}},
    {"min_average_gauge", {"Get %d%% gauge overall", ""}},
    {"max_overall_errors", {"Get less than %d errors total", ""}},
    {"max_overall_nears", {"Get less than %d nears total", ""}},
    {"min_overall_crits", {"Get at least %d crits total", ""}},
    {"gauge_carry_over", {"Gauge does not reset", ""}},
};
    ...
    int add)
{
    ...
    assert(ChallengeDescriptionStrings.Find(stringName)->first.find("%d") != String::npos || ChallengeDescriptionStrings.Find(stringName)->first.find("%u") != String::npos);
    assert(!isOverride || ChallengeDescriptionStrings.Find(stringName)->first.find("%d") != String::npos || ChallengeDescriptionStrings.Find(stringName)->first.find("%u") != String::npos);
