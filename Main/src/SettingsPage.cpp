...
bool SettingsPage::FloatSetting(GameConfigKeys key, const std::string_view& label, float min, float max, float step)
{
    ...
    if (step >= 0.01f)
    {
        ...
        if (incPerPixel >= step / 2)
            incPerPixel = step * Math::Round(incPerPixel / step);
...
protected:
    void m_createNewProfile(bool valid, char* data)
    {
        ...
        if (profile == "." || profile == "..")
            return;
        if (profile[0] == ' ' || profile[profile.length() - 1] == ' ' || profile[profile.length() - 1] == '.')
            return;
...
static inline void RenderButton(NVGcontext* vg, const struct nk_rect& rect, bool forcePortrait, Ref<TextRes> textRes, bool activated, const Vector2i& mousePos)
{
    ...
    struct NVGcolor bgColor = activated ? nvgRGB(60, 60, 60) : HitCheck(rect, mousePos) ? nvgRGB(128, 128, 128)
        : nvgRGB(15, 15, 15);
...
void SettingsPageCollection::ProcessTabHandleMouseHover(const Vector2i& mousePos)
{
    ...
    {
        if (CompareLine(m_prevMousePos, { m_pageContentRegion.x, m_pageContentRegion.y }, mousePos) <= 0 && CompareLine(m_prevMousePos, { m_pageContentRegion.x, m_pageContentRegion.y + m_pageContentRegion.h }, mousePos) >= 0)
        {
...
int SettingsPageCollection::GetPageIndFromMousePos(const Vector2i& mousePos) const
{
    ...
    if (!HitCheck(m_pageHeaderRegion, mousePos))
        return -1;
