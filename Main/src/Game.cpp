...
public:
    ...
    virtual bool AsyncLoad() override
    {
        ...
        if (IsChallenge() && m_speedMod == SpeedMods::CMod && !m_challengeManager->GetCurrentOptions().allow_cmod.Get(true))
        ...
        if (m_chartIndex)
        {
            ...
            for (ScoreIndex* score : m_chartIndex->scores)
            {
                ...
                if (m_replayForPlayback)
                {
                    ...
                    if (m_replayForPlayback->filePath == score->replayPath || (si.IsInitialized() && si.MatchesScore(score)))
                    {
                ...
                if (!replay)
                {
                    replay = Replay::Load(
                        score->replayPath,
                        (index == 0 && !m_replayForPlayback) ? Replay::ReplayType::Normal : Replay::ReplayType::NoInput);
                }
...
#define NVG_FLUSH()                                        \
    ...
    void BeginAfterGameTransition()
    {
        ...
        if ((m_manualExit && g_gameConfig.GetBool(GameConfigKeys::SkipScore) && m_multiplayer == nullptr && m_challengeManager == nullptr) || (m_manualExit && m_demo))
        {
    ...
    void m_OnEndRun(bool success)
    {
        ...
        if (m_playOptions.decSpeedOnFail && !success)
        {
            ...
            if (speedPercentage < minSpeedPercentage)
        ...
        if (m_playOptions.enableMaxRewind)
        {
            ...
            else
            {
                ...
                if (m_playOptions.range.begin < maxRewind)
                    m_playOptions.range.begin = maxRewind;
            }
        }
    ...
    virtual void RenderDebugHUD(float deltaTime)
    {
        ...
        // Playback info
        {
            ...
            textPos.y += RenderText(Utility::Sprintf(
                ...
                textPos, m_playOptions.playbackSpeed < 1.0f ? Color::Red : Color::Yellow)
                .y;
        ...
        // Input and scoring info
        {
            ...
            textPos.y += RenderText(Utility::Sprintf(
                ...
                textPos, m_scoring.hitWindow <= HitWindow::NORMAL ? Color{ 1.0f, 1.0f, 0.5f, 1.0f } : Color::Red)
                .y;
        ...
        // Camera and rendering info
        {
            ...
            textPos.y += RenderText(Utility::Sprintf("Roll: %f(x%f) %s",
                m_camera.GetRoll(), m_rollIntensity, m_camera.GetRollKeep() ? "[Keep]" : ""),
                textPos)
                .y;
    ...
    void MakePracticeSetup()
    {
        ...
        case GameFailCondition::Type::Score:
            m_playOptions.failCondition = std::make_unique<GameFailCondition::Score>(g_gameConfig.GetInt(GameConfigKeys::DefaultFailConditionScore));
            break;
        case GameFailCondition::Type::Grade:
            m_playOptions.failCondition = std::make_unique<GameFailCondition::Grade>(static_cast<GradeMark>(g_gameConfig.GetInt(GameConfigKeys::DefaultFailConditionGrade)));
            break;
        case GameFailCondition::Type::Miss:
            m_playOptions.failCondition = std::make_unique<GameFailCondition::MissCount>(g_gameConfig.GetInt(GameConfigKeys::DefaultFailConditionMiss));
            break;
        case GameFailCondition::Type::MissAndNear:
            m_playOptions.failCondition = std::make_unique<GameFailCondition::MissAndNearCount>(g_gameConfig.GetInt(GameConfigKeys::DefaultFailConditionMissNear));
            break;
        case GameFailCondition::Type::Gauge:
            m_playOptions.failCondition = std::make_unique<GameFailCondition::Gauge>(g_gameConfig.GetInt(GameConfigKeys::DefaultFailConditionGauge));
            break;
        default:
    ...
    void m_InitPracticeSetupDialog()
    {
        ...
        m_practiceSetupDialog->onSetMapTime.AddLambda([this](MapTime time)
            { if (m_isPracticeSetup) { m_paused = true; JumpTo(time); } });
        m_practiceSetupDialog->onSpeedChange.AddLambda([this](float speed)
            { if (m_isPracticeSetup) { m_playOptions.playbackSpeed = speed; ApplyPlaybackSpeed(); } });
    ...
    {
        if (m_playOptions.range.begin > 0)
            return true;
    ...
    virtual bool IsStorableScore() override
    {
        ...
        if (m_playOptions.playbackSpeed < 1.0f)
            return false;
        ...
        if (!(m_scoring.hitWindow <= HitWindow::NORMAL))
            return false;
