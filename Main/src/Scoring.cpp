            ...
            else if (delta >= 0)
            {
                ...
                else if (tick->HasFlag(TickFlags::Laser))
                {
                    ...
                    if (tick->HasFlag(TickFlags::Slam))
                    {
                        ...
                        if (autoplayInfo.autoplay || (dirSign == inputSign && delta <= hitWindow.slam / 2) || tick->HasFlag(TickFlags::Processed))
                        {
                            m_TickHit(tick, buttonCode);
                            HitStat* stat = new HitStat(tick->object);
                            stat->time = tick->time;
...
void Scoring::m_UpdateGauges(ScoreHitRating rating, TickFlags flags)
{
    ...
    bool isLong = (flags & TickFlags::Hold) != TickFlags::None || ((flags & TickFlags::Laser) != TickFlags::None && (flags & TickFlags::Slam) == TickFlags::None);
...
bool Scoring::m_IsBeingHeld(const ScoreTick* tick) const
{
    ...
    if (m_input && m_input->GetButton((Input::Button)index))
    {
        ...
        if (!heldObject || heldObject->type != ObjectType::Hold)
            return false;

        while (obj)
        {
            if (obj == (const HoldObjectState*)heldObject)
                return true;
            obj = obj->prev;
        }
    ...
    if (tick->HasFlag(TickFlags::Start) || !tick->HasFlag(TickFlags::End))
        return false;
    if (!m_prevHoldHit[index])
        return false;

    // b) The last button release happened inside the 'near window' for the end of this hold object.
    if (obj->time + obj->duration - m_buttonReleaseTime[index] - static_cast<MapTime>(m_inputOffset) > hitWindow.hold)
        return false;
...
constexpr static uint32 CalculateScore(const uint32 currHitScore, const uint32 maxHitScore, const uint32 maxScore)
{
    if (maxHitScore == 0)
        ...
        return (uint32)(((double)currHitScore / (double)maxHitScore) * (double)maxScore);
}
