...
void GameplaySettingsDialog::InitTabs()
{
    ...
    {
        offsetTab->settings.push_back(CreateButton("Compute Song Offset", [this](const auto&)
            { onPressComputeSongOffset.Call(); }));
    }
    ...
    if (m_multiPlayerScreen == nullptr)
    {
        ...
        gameTab->settings.back()->setter.AddLambda([this](const auto& data)
            { this->ResetTabs(); });
    ...
    if (m_songSelectScreen != nullptr)
    {
        gameTab->settings.push_back(CreateButton("Replay Best Score", [this](const auto&)
            { onPressReplay.Call(); }));
        gameTab->settings.push_back(CreateButton("Autoplay", [this](const auto&)
            { onPressAutoplay.Call(); }));
        gameTab->settings.push_back(CreateButton("Practice", [this](const auto&)
            { onPressPractice.Call(); }));
    }
    ...
    judgeWindowTab->settings.push_back(CreateButton("Set to NORMAL", [](const auto&)
        { HitWindow::NORMAL.SaveConfig(); }));
    judgeWindowTab->settings.push_back(CreateButton("Set to HARD", [](const auto&)
        { HitWindow::HARD.SaveConfig(); }));
...
GameplaySettingsDialog::Setting GameplaySettingsDialog::m_CreateSongOffsetSetting()
{
    ...
    songOffsetSetting->setter.AddLambda([this](const auto& data)
        { onSongOffsetChange.Call(data.intSetting.val); });
