...
PracticeModeSettingsDialog::Tab PracticeModeSettingsDialog::m_CreateMainSettingTab()
{
    ...
    speedSetting->setter.AddLambda([this](const SettingData& data)
        { onSpeedChange.Call(data.intSetting.val == 100 ? 1.0f : data.intSetting.val / 100.0f); });
    speedSetting->getter.AddLambda([this](SettingData& data)
        { data.intSetting.val = Math::RoundToInt(m_playOptions.playbackSpeed * 100); });
    mainSettingTab->settings.emplace_back(std::move(speedSetting));

    mainSettingTab->settings.emplace_back(CreateButton("Start practice", [this](const auto&)
        { onPressStart.Call(); }));
    mainSettingTab->settings.emplace_back(CreateButton("Exit", [this](const auto&)
        { onPressExit.Call(); }));
...
PracticeModeSettingsDialog::Tab PracticeModeSettingsDialog::m_CreateLoopControlTab()
{
...

    {
        ...
        incSpeedOnSuccess->setter.AddLambda([this](const SettingData& data)
            { if (data.boolSetting.val) m_playOptions.loopOnSuccess = true; });
        ...
        incSpeedAmount->getter.AddLambda([this](SettingData& data)
            { data.intSetting.val = Math::RoundToInt(m_playOptions.incSpeedAmount * 100); });
        ...
        incStreak->setter.AddLambda([this](const SettingData&)
            { m_playOptions.incSpeedOnSuccess = m_playOptions.loopOnSuccess = true; });
...

    {
        ...
        decSpeedOnFail->setter.AddLambda([this](const SettingData& data)
            { if (data.boolSetting.val) m_playOptions.loopOnFail = true; });
        ...
        decSpeedAmount->getter.AddLambda([this](SettingData& data)
            { data.intSetting.val = Math::RoundToInt(m_playOptions.decSpeedAmount * 100); });
        ...
        minSpeed->getter.AddLambda([this](SettingData& data)
            { data.intSetting.val = Math::RoundToInt(m_playOptions.minPlaybackSpeed * 100); });
...

    {
        ...
        maxRewindMeasure->setter.AddLambda([this](const SettingData& data)
            { if (data.boolSetting.val) m_playOptions.loopOnFail = true; });
...
std::unique_ptr<GameFailCondition> PracticeModeSettingsDialog::m_CreateGameFailCondition(GameFailCondition::Type type)
{
    ...
    case GameFailCondition::Type::Score:
        return std::make_unique<GameFailCondition::Score>(m_condScore);
    case GameFailCondition::Type::Grade:
        return std::make_unique<GameFailCondition::Grade>(m_condGrade);
    case GameFailCondition::Type::Miss:
        return std::make_unique<GameFailCondition::MissCount>(m_condMiss);
    case GameFailCondition::Type::MissAndNear:
        return std::make_unique<GameFailCondition::MissAndNearCount>(m_condMissNear);
    case GameFailCondition::Type::Gauge:
        return std::make_unique<GameFailCondition::Gauge>(m_condGauge);
    default:
        return nullptr;
    }
}
